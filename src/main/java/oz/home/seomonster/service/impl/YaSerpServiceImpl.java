...
public class YaSerpServiceImpl extends BaseSerpService {
    ...
    @Value("${seomonster.serp.yaSerpItemClassname}")
    public String YA_SERP_ITEM_CLASSNAME;

    @Value("${seomonster.serp.captchaTagClassName}")
    ...
    public Captcha extractCaptcha(Document doc) {
        ...
        try {
            String imageUrl = doc.select(CAPTHCHA_TAG).first().attr("src");
